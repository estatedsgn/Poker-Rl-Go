...
- `evaluate_pool.py` — ранжирование пула чекпоинтов по средней выплате на руку (+ std и 95% CI по нескольким seed).
- `run_seed_sweep.py` — запуск серии synthetic-train прогонов по нескольким seed для проверки стабильности.
...
- Можно включить авто-оценку чекпоинтов во время обучения через `--eval-hands`, `--eval-seed-start`, `--eval-num-seeds` (если `eval_num_seeds>0` и `eval_hands>0`, в финальные metrics попадут `best_eval_*`).
- Выбор лучшего чекпоинта настраивается через `--selection-metric mean|lcb` и `--selection-ci-penalty`; доступен ранний останов `--early-stop-patience-checkpoints`.
...
Для многосидовой оценки (mean/std/95% CI):

```bash
python evaluate.py --checkpoint checkpoints_rlcard/policy_step_000200.pt --hands 500 --seed-start 42 --num-seeds 5
```
...
### Оценка пула чекпоинтов по нескольким seed

```bash
python evaluate_pool.py --checkpoints-dir checkpoints_rlcard --hands 300 --seed-start 42 --num-seeds 5
# более консервативный ранк: mean - ci95
python evaluate_pool.py --checkpoints-dir checkpoints_rlcard --hands 300 --seed-start 42 --num-seeds 5 --rank-metric lcb --ci-penalty 1.0
```


### Быстрый sweep по seed (synthetic backend)

```bash
python run_seed_sweep.py --seeds 42,43,44 --steps 50 --out-root sweep_runs
```


### Профили конфигурации

- `configs/default.json` — базовый учебный профиль.
- `configs/train_debug.json` — быстрый профиль для локальной отладки (короткий run).
- `configs/train_working.json` — рабочий профиль для более длинного запуска с checkpoint-eval.

Примеры:

```bash
python train.py --config configs/train_debug.json
python train.py --config configs/train_working.json
```
...
`train.py` сохраняет:
- `metrics.jsonl` — построчные метрики по шагам/чекпоинтам,
- `summary.json` — финальный summary по запуску.

Оба файла пишутся в `out_dir`.
