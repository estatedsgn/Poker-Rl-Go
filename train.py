...
class TrainConfig:
    ...
    eval_hands: int = 0
    eval_seed_start: int = 42
    eval_num_seeds: int = 0
    selection_metric: str = "mean"  # mean | lcb
    selection_ci_penalty: float = 1.0
    early_stop_patience_checkpoints: int = 0
...
def _selection_score(mean_payoff: float, ci95_half_width: float, metric: str, ci_penalty: float) -> float:
    if metric == "mean":
        return mean_payoff
    if metric == "lcb":
...
def _evaluate_checkpoint_over_seeds(cfg: TrainConfig, ckpt_path: Path) -> Dict[str, float]:
    from evaluate import run_evaluation_over_seeds

    seeds = [cfg.eval_seed_start + i for i in range(cfg.eval_num_seeds)]
    ...
    return {
        "eval_mean_payoff": summary.mean_payoff,
        "eval_std_payoff": summary.std_payoff,
        "eval_ci95": summary.ci95_half_width,
        "eval_num_seeds": float(summary.num_seeds),
    }
...
def run_training(cfg: TrainConfig) -> Dict[str, float]:
    ...
    for step in range(1, cfg.steps + 1):
        ...
        if step % cfg.checkpoint_interval == 0 or step == cfg.steps:
            ...
            latest_metrics["checkpoint_path"] = str(ckpt_path)
            latest_metrics["checkpoint_index"] = float(len(checkpoints))

            if cfg.eval_num_seeds > 0 and cfg.eval_hands > 0:
                eval_metrics = _evaluate_checkpoint_over_seeds(cfg, ckpt_path)
                latest_metrics.update(eval_metrics)

                current_score = _selection_score(
                    eval_metrics["eval_mean_payoff"],
                    eval_metrics["eval_ci95"],
                    metric=cfg.selection_metric,
                    ci_penalty=cfg.selection_ci_penalty,
                )
                latest_metrics["selection_score"] = current_score

                if current_score > best_eval_score:
                    best_eval_score = current_score
                    best_eval_mean = eval_metrics["eval_mean_payoff"]
                    best_eval_ci95 = eval_metrics["eval_ci95"]
                    best_eval_step = float(step)
                    checkpoints_since_improve = 0
                else:
                    checkpoints_since_improve += 1

                latest_metrics["checkpoints_since_improve"] = float(checkpoints_since_improve)

                if (
                    cfg.early_stop_patience_checkpoints > 0
                    and checkpoints_since_improve >= cfg.early_stop_patience_checkpoints
                ):
                    latest_metrics["early_stop_triggered"] = 1.0
                    latest_metrics["early_stop_step"] = float(step)
    ...
    if best_eval_step > 0:
        latest_metrics["best_eval_mean_payoff"] = best_eval_mean
        latest_metrics["best_eval_ci95"] = best_eval_ci95
        latest_metrics["best_eval_step"] = best_eval_step
        latest_metrics["best_selection_score"] = best_eval_score

    _write_summary(out_dir / "summary.json", latest_metrics)
...
def parse_args() -> TrainConfig:
    ...
    p.add_argument("--eval-hands", type=int, default=0)
    p.add_argument("--eval-seed-start", type=int, default=42)
    p.add_argument("--eval-num-seeds", type=int, default=0)
    p.add_argument("--selection-metric", type=str, choices=["mean", "lcb"], default="mean")
    p.add_argument("--selection-ci-penalty", type=float, default=1.0)
    p.add_argument("--early-stop-patience-checkpoints", type=int, default=0)
    ...
    return TrainConfig(
        ...
        selection_ci_penalty=args.selection_ci_penalty,
